...
> **仓库说明**：本文件已针对 `method-engineering` 仓库填写。治理核心位于 `docs/`，Skills 包位于 `skills/`；尚未创建具体工作区，工作区路径使用 `docs/workspaces/workspace-<NNN>-<slug>/` 这一命名模式。
...
| 目标树与状态 | `docs/workspaces/workspace-<NNN>-<slug>/goal-tree.md` | **必读、必更新** |
...
| 核心模板 | `docs/templates/`（或 `docs/templates`） | **必备**；创建五件套与 workspace 上下文 |
| 工作区与共享资料协议 | `docs/workspaces/workspace-<NNN>-<slug>/workspace.md`、`docs/architecture/workspace-protocol.md` | workspace.md 存在时必读；protocol **必备**；目标状态仍以该工作区根为准 |
...
- 若项目提供独立核心模板层，优先从 `docs/templates/goal-folder/` 复制；否则从 `docs/templates/goal-folder` 复制（常见：`skills/templates/goal-folder/`；包目录名可能不是 `skills`）。
...
**「总路线图」**（口语）= 愿景层组合编排索引 `docs/vision/roadmap.md`，**不是**目标层纲领路线图；两者不得互相替代。
...
- **代码布局（默认策略）**：
  ...
  - 若项目已约定子目录（如 `web/`、`app/`、`services/`）：按该约定；`.` 仅在有约定时填写。
...
4. **Skills 包（可选）**：`skills`

## 9b. Skills 主入口（若已安装本包）

- **实现主路径**：`skills/prompts/00-govern-orchestrator.md` → **`/govern`**。  
  扫描 → 意见台账 → 分类 → P-004 裁决 → 提议 → 确认 → 原语 `01`～`04`。
- **决策层**：**`/vision`** → `skills/prompts/06-vision-orchestrator.md`（Charter / VP / Vision Review / re-align；冷启动优先）。
- **交叉审计**：`skills/prompts/05-independent-audit.md` → **`/audit`**（只出意见，不改 status；响应归 `/govern`）。
...
- 文档说明：`docs/README.md`
- 目标树：`docs/workspaces/workspace-<NNN>-<slug>/goal-tree.md`
- Root Goal：`docs/workspaces/workspace-<NNN>-<slug>/GOAL-001-<root-slug>/00-meta.md`
- 核心模板目录：`docs/templates`（若项目采用独立核心层）
