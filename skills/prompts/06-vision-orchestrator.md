...
7. **工作区对齐（只读）**：各 `{governance_root}/workspaces/workspace-*/workspace.md` 的 `plan_refs`/`primary_plan`/`vision_role`（不混合多区目标正文）；旧直属 `{governance_root}/workspace-*/workspace.md` 只报告迁移阻断，新旧并存 fail closed。
