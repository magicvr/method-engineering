...
**记录位置**：路线图应写在该目标的 `00-meta.md` 或 `01-decision.md` 中，并随进展更新。**范围限定**：「纲领路线图」只属于某个 Root/大目标；愿景层没有也不得出现同名的可执行纲领路线图（愿景层只有**组合编排**与 VP 的方向级阶段结构，判定见 §6.4 谓词）。
...
| 名称 | 层 | 写什么 | 不写什么 |
|------|----|--------|----------|
| **组合编排** | 愿景 | VP 索引、波次先后/并行、lead 区；VP status 只可作**显式标注的派生投影** | progress%、goal-tree、finding、作为 VP status 的第二权威 |
| **纲领路线图** | Root/大目标 | 纲领阶段与先后、完成标记 | 细任务流水；愿景层工件不得同名为「纲领路线图」 |
| **阶段计划** | 目标内 | 某阶段方案与实施安排（多在 `01-decision`） | 独立生命周期实体、第二套状态源 |
| **意图（VP）** | 愿景层权威 | 已落盘 `VP-*.md`；意图、方向级退出判据、`vision_ref`、工作区绑定表、关门记录 | 目标五件套、`parent`、Goal status（`done`/`draft`/`blocked`/`cancelled`）、progress% 权威、第二套审计台账 |
| **子目标** | 实现层 | 工作区内 `GOAL-NNN-<slug>` 五件套节点；`parent` = 父**目标**完整 id | 把 VP/Charter 当 `parent`；把 VP 写成可执行子目标 |
| **VP 内阶段结构** | 愿景层（受限） | 波次内**方向级**阶段，用于说明先后与退出方向（如「方向级路线图」节） | **可执行纲领路线图**：细任务阶段、完成标记、子目标编号、Goal status、progress% |

意图**草案**可暂存会话/决策草稿，**无日历死线**；一旦挂 `plan_refs`/`primary_plan` 或宣称服务某意图，必须已有 VP 文件。

**判定谓词 ·「某段路线图/阶段内容属于哪一层」（最小充分条件）**：

满足下列**任一**条即判为**层级错位**（须按上层改写或升级修订，不得静默保留）：

| # | 最小充分条件 | 归属 |
|---|--------------|------|
| 1 | 内容授权或改变**决策**（目的、边界、非目标、波次先后、lead 区） | 愿景层：Charter 或组合编排 |
| 2 | 内容是**已落盘 VP** 的意图、方向级退出判据、绑定/关门记录 | 愿景层：该 VP 正文 |
| 3 | 内容划分**本工作区某个 Root/大目标**的纲领阶段与先后 | 实现层：该目标 `00-meta.md` / `01-decision.md` |
| 4 | 内容安排**某个阶段内**的方案与实施顺序 | 实现层：阶段计划（非树节点） |
| 5 | 内容是**可独立验收**的交付节点，需要 `parent`、五件套、状态与审计 | 实现层：子目标 |
| 6 | 内容把 VP、Charter 或愿景文件当作目标节点 / `parent` / Goal 状态源 | **层级错位**（fail closed） |
| 7 | 内容在愿景层写出可执行纲领阶段、子目标编号或 Goal status / progress% | **层级错位**（fail closed） |
| 8 | 内容在目标层复写第二套愿景边界（超出短 `serves_summary` + 链接） | **层级错位**（fail closed） |

**判定对象是职责与权威，不是节点数量**：一区一 Root 一 VP、多 VP 绑同一区、0 区 `active` VP、把实施方案写进 `01-decision` 都是**合法**结构；数量相同或不同都不证明分层成立或失败。拿不准 → 按 P-004 询问。

**术语映射**：外部/消费方口语中的「**总路线图**」「愿景路线图」指愿景层组合编排索引（`{governance_root}/vision/roadmap.md`），**不是**目标层纲领路线图；后者只属于某个 Root/大目标，位于其 `00-meta.md` 或 `01-decision.md`。两者不得互相替代，也不得把组合编排表写进目标文件。
...
**VP（意图）**：意图说明；方向级退出判据；`vision_ref`；工作区绑定表；可关门记录（closed 时）。证据在工作区，不在 vision 写 progress%。**受限**：可写方向级阶段结构（先后与退出方向），**不得**写可执行纲领阶段、子目标编号、Goal status 或 progress%；状态与跟踪权威在 VP 自身的 frontmatter（`status` / `vision_ref` / `lead_workspace`），组合编排索引中的同名列只能是**显式标注的派生投影**（见 [alignment.md §0.3](../vision/alignment.md)）。
