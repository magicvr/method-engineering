...
本 VP 所称**消费方 / 下游需求方**，是直接参与真实下游实践、提出方法需求并接收响应的个人或团队。**试跑仓库**是承载该实践或其请求记录的真实项目仓库；它是否安装了 goal-governance 治理包，不决定其是否属于本 VP 的试跑对象。这里的消费方/试跑仓库，不等同于 `alignment.md` 与消费核对表中“安装并使用 goal-governance 包的消费项目”。本 VP 的退出验证采用双方在真实实践仓库共同进行的协议联调演练，场景内容明确标为合成且不含敏感资料；演练不代表真实需求处理。
...
- 至少一个真实消费仓与方法工程共同完成一条共享追踪线上的协议联调演练；双方实际消费对方交接内容，完成信号、回执/澄清、演练专用范围与授权、响应交接、收件反馈与结束。
- 联调演练只验证协议的可理解性、可执行性与交接连贯性，不证明具体方法普遍有效，也不代表协议已适用于所有消费仓。
...
5. 与至少一个真实下游需求方在其真实实践仓库中共同开展协议联调演练，使用明确标注的合成、非敏感内容，记录并修正交接歧义；不要求构建领域方法。
...
- 为所有消费仓建立统一仓库结构，或声称一次联调演练证明协议普遍适用。
- 交付某个特定领域方法、保证消费方必须接受响应，或证明响应的普遍有效性。
- 替代 VP-001 的运行记录状态、目标治理状态、信息门禁或审计台账。
- 将演练材料冒充真实需求、真实方法响应或方法有效性证据；不为合成内容创建虚假 runtime-records/WRK-* 记录。真实需求另循既有运行协议处理。
...
5. 至少一个真实下游需求方与方法工程在真实实践仓库中共同完成协议联调演练：使用合成且非敏感内容，沿一条共享追踪线实际消费对方交接，覆盖“信号 → 回执/澄清 → 演练专用范围与授权 → 响应交接 → 消费方收件与反馈 → 结束”。两侧各自 walkthrough 后拼接、只有生产侧内部记录、治理包安装演练均不能替代双边集成证据；不要求交付真实领域方法。若同一人承担消费方与方法工程角色，须记录角色切换；证据只证明本次对接流程经过演练，不声称独立团队达成共识。
6. 试跑证据能复核协议的实际可读性与交接结果；发现的歧义已经修正或被明确列为有界 residual，并且残余不遮蔽本 VP 声称完成的范围。

VP-002 关门须链接双方共同联调的实施与结项证据，明确内容为演练及证据所证明的协议范围。实际参与方、共享追踪、交接消费或演练授权尚未落实时保持未关门。真实方法构建需求不作为本 VP 的退出前提。

## 实施前待确认

参与方与真实实践仓库已在工作区确认；联调场景、双方责任、共享追踪和证据核对方式，以及下游选定的演练材料路径、格式、工具与写入授权仍须在实施前明确，信息门禁由工作区登记。已有真实需求保留其待判定记录；本次范围修订不接受其处理承诺，也不自动创建后继 VP。
