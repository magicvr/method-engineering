...
| VR-005 | 2026-09-26 | editorial | 按用户指令登记 VP-002 已有界 `closed`，并更新 Charter 关系说明与组合索引投影；未改变 Charter 目的、边界、非目标、版本或 `primary_workspace`，也不归档 workspace-002。 | Charter 与 VP 当前状态说明 | applied |
| VR-006 | 2026-09-26 | editorial | 按用户确认登记 VP-003 已落盘即 `active` 并绑定新工作区；将 `primary_workspace` 由 `workspace-002-consumer-response-protocol` 改为 `workspace-003-world-model-method-and-tools`，同步 `workspaces.md` 与 workspace-002 的 `vision_role`（`primary`→`delivery`），使 vision 层唯一 `primary` 指向实际承载现行 active VP 的工作区；并登记 VP-003 承接真实需求 `WRK-002-world-model-method-and-tools`。未改变 Charter 目的、边界、非目标、版本或 `vision_ref`。 | Charter `primary_workspace` 字段；VP-003 与工作区绑定说明；工作区角色元数据 | applied |
