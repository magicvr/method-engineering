...
Root 的 P-001 纲领路线图为 **R1 → R2/R3 → R4**。R1 已完成（2026-09-26）：适用对象、退出形态、限额与责任、工具边界均已冻结（`D-002` / `E-003`）。**R2 进行中**，由子目标 `GOAL-002` 承载（工作包 W1～W4，**1/4**：W1 条款映射与差异登记已完成；**W2 整改中**）；R3 经复核无独立形成工作，待交付材料落定后确认；R4 未开始。Root 派生 `progress: 25%`（1/4）——R2 未完成，故不计完成。progress 不推导 `done`。
...
| `GOAL-002-r2-method-working-version` | 形成《世界模型构建方法（工作版）》与两个最小结构 | `GOAL-001-world-model-method-and-tools` | active | 25% | 承载 Root R2 与 R3 的落实。工作包 **W1 → W2 → W3 → W4**（1/4）：**W1 已完成**（`D-004` / `E-004`：17 行条款级映射 + 附表 A/B + 差异 `E1`～`E3` + 候选 `C1`～`C8`）。W4 判据 = 用户 2026-09-26 提供的真实问题（星际时代修真个体伟力及其社会影响）。**执行主体与协作约束（`D-002` → 修正于 `D-003` / `E-003`）**：主题、取舍与最终裁决属于创作者，助手**不得代劳**但**应尽可能协助**；界线 = 「助手给建议 + 创作者裁定 = 协助」对「助手直接定结论并被采用 = 代劳」；要求**建议与裁定分开留痕**。`I-201` verified；`I-205`（`C2` / `C3` / `C6` 裁定）**已裁定并 verified**（[`D-008`](GOAL-002-r2-method-working-version/01-decision/D-008-i205-adjudication.md)）；`I-202` / `I-203` / `I-204` open。**W2 整改中**（草稿 [`v0.3`](GOAL-002-r2-method-working-version/attachments/world-model-method-working-version-v0.3.md)；`v0.1` / `v0.2` 保留）：`v0.2` 曾被记为完成，但独立审 [`A-005`](GOAL-002-r2-method-working-version/03-audit/A-005-w2-independent-review.md) 判其退出条件**两条行不满足**（总流程有路径跳过「完成裁决」；`E2` 的 §12 边界节缺失），**完成标记已按 [`D-010`](GOAL-002-r2-method-working-version/01-decision/D-010-a005-response.md) 收回**。`v0.3` 已整改 `F-001`～`F-003`（第 2 章步 ⑦ 成为**必经步**并分开写明**五条出口**；第 7 章新增**条件敏感性最低核对**；第 8 章新增 **§12 边界节**，不新开章），**修正已落盘、待独立 closure check**。**两个最小结构尚未形成**（W3 待办）。**审计状态**：`A-001`～`A-003`（W1，independent）已闭合；`A-004`（self，针对 `v0.1`）已整改；**`A-005`（independent，针对 `v0.2` 并复核 `A-004` 自闭合）开放 required 3 条，状态「修正已落盘、待 closure check」**，`F-004` 已按创作者裁定处理（机制为「条件关键项」，能力声明须收窄）。**闭审确认前不得推进 W3。** |
