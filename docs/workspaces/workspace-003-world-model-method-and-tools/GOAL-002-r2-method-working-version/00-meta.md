...
- [x] **W2** 方法主文档形成 **完成**（2026-09-26，草稿 **`v0.2`**，[`attachments/world-model-method-working-version-v0.2.md`](attachments/world-model-method-working-version-v0.2.md)）：经 [`A-004`](03-audit/A-004-w2-draft-self-review.md) 整改并按 W2 退出条件逐项复核通过。见 [`D-009`](01-decision/D-009-a004-response.md) / [`E-009`](02-execution/E-009-w2-draft-v0-2.md)。
...
| **W2** | 方法主文档形成 | **完成**（2026-09-26，草稿 `v0.2`） | 正文按 R1 冻结九章结构齐备（`C6` 另增「总流程」章，共 10 章），含 §13 映射表、**逐章**适用条件 / 已知边界 / 未决事项；**每一章标明人机协作位**（判断点 / 助手可协助 / 不得越过 / 留痕），并给出**建议与裁定分开的留痕要求**；草稿落 [`attachments/world-model-method-working-version-v0.2.md`](attachments/world-model-method-working-version-v0.2.md)（`v0.1` 保留为历史）。经自审 [`A-004`](03-audit/A-004-w2-draft-self-review.md) 整改并复核通过；见 [`D-008`](01-decision/D-008-i205-adjudication.md) / [`D-009`](01-decision/D-009-a004-response.md) / [`E-009`](02-execution/E-009-w2-draft-v0-2.md)。 |
...
`progress: 50%` 由上表 4 个工作包检查点中的 2 个完成项等权计算（2/4）。progress 只作展示。它不放行阶段、不关闭 finding、不覆盖信息门禁，也不推导 `status: done`。Root 的 `progress` 不由本表推导，两者各自按其检查点独立计算。
...
**门禁现状（2026-09-26）**：`I-201`（真实问题）已关闭；`I-205`（`C2` / `C3` / `C6` 裁定）**已裁定并置 `verified`**（[`D-008`](01-decision/D-008-i205-adjudication.md)）；审计门禁**已解除**（`A-003` closure check `pass`，见 [`D-007`](01-decision/D-007-w1-freeze.md)）。剩余 required：`I-202`（走查记录是否并入交付包，最晚 W4 结束前）、`I-203`（草稿升格路径，最晚 R4 交付前）、`I-204`（W4 核对协作安排，最晚 W4 开始前）。**三者均不阻断 W2 的阶段审视与 W3。**
...
> **并行会话产物已删除**：工作区中 `attachments/independent-review-request-A-005.md` 由**并行会话**产出、引用一份本仓并不存在的 `A-004-w2-self-review.md`；经用户 2026-09-26 裁决**删除**（该文件**从未提交**，删除只影响工作区）。其两处有效发现已由本自审 `F-006` 与 `F-001` 的补充关闭要求吸收。
