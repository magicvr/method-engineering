...
**门禁现状（2026-09-26，`A-005` 到达后）**：**信息项**：`I-201` / `I-205` verified；`I-202`（W4 结束前）/ `I-203`（R4 交付前）/ `I-204`（W4 开始前）open——**均不到期**。**审计门禁（新）**：独立审 [`A-005`](03-audit/A-005-w2-independent-review.md)（`conditional`）对草稿 `v0.2` 提出 **3 条 required**（`F-001` 第 2 章"已支持"与"非机制缺口"两条路径跳过「完成裁决」；`F-002` §13 第 7 项映射落点核对不上、缺条件敏感性最低核对；`F-003` 冻结输入 `E2` 要求的 §12 边界节缺失）与 **1 条 recommended**（`F-004` 「关键项」默认清单未含第⑤项机制本身）。**三条 required 闭合前不得推进 W3**；W2 完成标记的维持/收回待用户裁决——**裁决前该标记不作为 W3 放行依据**。
