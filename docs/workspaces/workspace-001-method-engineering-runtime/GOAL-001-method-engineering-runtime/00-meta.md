...
- [ ] 运行模型明确需求信号、澄清与接受、响应选择、验证、交付、反馈、退出及 IDLE 之间的语义、状态转换、责任交接和追踪关系。
- [ ] 运行模型能够记录选择、复用、裁剪、组合、修改、新建或确认无需方法变更等最小响应路径，并区分对象问题、方法问题和运行机制问题。
- [ ] 由运行责任反推出的最小工作对象、状态、目录和流程文档已落盘；每个保留对象都有明确运行责任，不产生第二套状态源。
- [ ] bounded walkthrough 完整走通机制自身的需求进入、状态转换、责任交接、响应选择、验证、交付、反馈和退出，并明确标注其不是真实 Case、不证明领域方法有效性。
- [ ] 工作区和 Root 的实施、验证、审计与结项证据可追踪到 VP-001 的方向级退出判据。
...
| **S2** | 落盘最小工作机制 | 进行中 | 根据 S1 的运行责任确定最小工作对象、状态承载、记录边界、仓库结构和必要流程文档；I-002 关闭；未引入无法由运行责任推出的机制。 |
| **S3** | 机制 walkthrough 与交接 | 未开始 | 完成一次明确标注为机制验证的 bounded walkthrough，记录其内部连贯性、剩余问题和修正；不把 walkthrough 写成真实 Case 或方法有效性证据，并形成 Root/VP 后续结项所需证据。 |

阶段按 S1→S2→S3 串行；同一阶段内若出现具有独立范围、依赖或交付证据的工作，才创建平铺子目标。

## 派生进度展示

`progress: 33%` 由上方 3 个纲领阶段等权计算（已完成 1 / 3）。该值仅作展示，不放行阶段、不关闭 finding、不覆盖信息门禁，也不自动推导 `done`。
...
| I-002 | required | 支撑运行模型所必需的最小工作对象、记录边界、目录承载和流程文档集合。 | 实施、S2 退出 | S2 | 从已冻结运行责任反推对象和仓库结构，逐项说明保留理由。 | open | 不延期；S2 结束前必须处理。 | 待完成责任→对象推导。 |
