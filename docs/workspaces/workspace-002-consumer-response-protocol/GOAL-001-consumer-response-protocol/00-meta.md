...
- [ ] 下游拥有交付目录、格式与工具最终选择权，可在提交或澄清时确定；若响应交付需向下游仓库写入材料，须在「已接受」承诺及任何写入前确认路径、格式、工具和授权；I-002 范围内的 method-engineering 运行记录依 D-004/D-006 承载，不由未定的响应交付路径阻挡；无需文件交付时约定沟通渠道，不把完整路径作为首次提交强制字段。
...
| **R3** | 执行并审视真实端到端试跑 | 进行中（信号待判定） | 在真实试跑仓库中对一条真实且获授权处理的需求完成端到端试跑，取得消费方收件回执与验收或具体异议；歧义已修正或列为有界 residual；I-003 `verified`；试跑验收与指南验证完成后，按用户确认的本仓共享路径升格指南唯一权威全文并核对引用，关闭 I-006，且在 Root 关门前完成；形成可支撑 VP-002 关门的工作区证据链。真实需求信号已登记为 WRK-001；具体处理授权与需求/Root 范围适配仍待确认，尚未进行实质处理。 |
...
| I-003 | required | 至少一条符合条件的真实方法需求及其来源、追踪信息、受理/授权状态。 | R3 试跑、VP-002 关门证据 | R3 | 在已授权范围内由消费方提出或转述一条真实需求，按 D-006 先在 I-002 范围内登记信号并分配 ID，再取得具体需求处理授权及「已接受」承诺后开展实质处理；登记、回执和澄清不构成接受或处理承诺。依 D-005 由下游选择响应交付目录、格式、工具；需要向下游仓库写入响应材料时，在接受承诺及任何写入前确认路径、格式、工具和授权；无文件交付时约定渠道。 | open | 真实信号已于 2026-09-26 按 D-006 登记为 `WRK-001-world-model-demand-method`；具体处理授权、接受承诺与可执行范围尚未确认，不得以模拟需求替代。 | [E-019](02-execution/E-019-r3-demand-signal-intake.md)；[运行主记录](../../../runtime-records/WRK-001-world-model-demand-method/record.md) |
...
| I-007 | required | R3 需求请求交付的世界模型构建方法工作版与 Root “不交付特定领域方法”非目标如何对齐，是否作为有界试跑响应或需要重新对齐目标范围。 | R3 处理接受与 Root 关门 | R3 接受处理承诺前 | 向用户展示范围冲突和可执行选项，记录裁决；若改动 Root/VP 方向或退出条件，先完成对应 Vision / Goal 重新对齐。 | open | 用户裁决前，WRK-001 保持「待判定」，不进行实质处理；责任人：用户；复核触发：用户决定采用哪种范围处理路径。 | R3 需求信号见 [E-019](02-execution/E-019-r3-demand-signal-intake.md)；Root 非目标见本文件「概述」。 |
...
- 与 VP-001 的关系：沿用其「已接受需求」/IDLE 权威与单一运行主记录语义，一般消费仓根目录所有权规则保持不变；本次 R3 试点按 D-004 的有界例外在 method-engineering 根 `runtime-records/<work-item-id>/` 承载。
