...
| 阶段 | 名称 | 状态 | 退出条件 |
|------|------|------|----------|
| **R1** | 冻结协议语义与运行记录衔接 | 未开始 | 双向协议语义（回执、澄清、受理/不受理、边界与限额、状态沟通、范围变化、退出、交付/收件/验收/异议区分、反馈分类与新需求重入）与 VP-001 的运行记录语义衔接方式冻结并落盘；I-004 `verified`；相关 required finding 为 0。 |
...
| I-004 | required | 双向协议语义（回执、澄清、受理、边界、交付/收件/验收/异议区分、反馈分类与重入）与项目根 `runtime-records` 单一主记录及事件追踪的具体衔接表达。 | R1 方案冻结 | R1 | 基于 VP-001 已确立的单一主记录与追加式事件语义逐项映射，并核对不产生第二状态源。 | open | 本轮尚未开始；R1 第一项工作。 | 待确认 |
...
本 Root 的实施审计模式尚未确定。按 scope 风险初步建议 **`cross`**：协议属跨边界/元规则类交付，R3 证据涉及真实外部参与方与敏感信息处理。进入 R1 实施前须由用户确认模式（`none` / `self` / `independent` / `cross`）及 independent provider，并写入 `01-decision`。在此确定前只做语义澄清与信息收集，不宣称阶段放行。
