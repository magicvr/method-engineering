...
- [ ] 下游拥有交付目录、格式与工具最终选择权，可在提交或澄清时确定；涉及仓库写入时在「已接受」承诺及任何写入前确认路径、格式、工具和授权；无需文件交付时约定沟通渠道，不把完整路径作为首次提交强制字段。
...
| I-003 | required | 至少一条符合条件的真实方法需求及其来源、追踪信息、受理/授权状态。 | R3 试跑、VP-002 关门证据 | R3 | 在已授权范围内由消费方提出或转述一条真实需求，按协议登记并取得处理授权；依 D-005 由下游选择交付目录、格式、工具，涉及仓库写入时在接受承诺及任何写入前确认路径、格式、工具与授权，无文件交付时约定渠道。 | open | 尚未获得；不得以模拟需求替代。 | 待确认 |
...
| I-005 | non-blocking | 运行记录的具体字段、模板、work-item ID、引用字段及沟通渠道等尚未确定的物理承载细节。 | 不阻断 R2 协议语义；具体建档与协作时核对适用细节 | R3 实际建档与逐案协作时 | 沿用既有 record.md + events.md 模式；I-003 真实需求授权后分配实际 work-item ID，并按需求确认字段、引用和渠道。 | open | 责任人：运行记录维护者与消费方；复核触发：真实需求授权及建档准备。尚未定案的细节不得写成已验证。 | D-002 第 5、11 项保留原信息项身份；[D-004](01-decision/D-004-trial-runtime-record-host.md) 仅确定本次试点宿主与目录模式，其余细节仍待逐案确定。 |
...
按 [D-003](01-decision/D-003-protocol-guide-lifecycle-and-promotion.md)，先在本目标 `attachments/consumer-response-protocol.md` 编写唯一协议草稿，再完成 self + 上下文独立 Codex Reviewer 的 **cross** 审计，最后取得消费方可读可执行确认。R2 涉及跨边界运行协议，依 P-003 采用 cross；草稿已落盘，A-006/A-007 均为 fail；本次修正待复审及消费方确认，5 条 required findings 仍为 open。独立 provider 沿用本会话指定的 Codex Reviewer；派发限制同上。R3 待真实需求范围明确后重新判定审计模式与 provider。R2 草稿按 D-005 纳入下游交付选择权及写入前确认要求；本次具体交付目录仍待随 I-003 真实需求明确。
