...
- [x] R3 试跑验收与指南验证完成后、Root 关门前，将协议指南唯一权威全文升格到用户确认的本仓共享运行文档路径，并核对引用；工作区仅保留治理证据与指向，不保留两份可编辑全文。最终路径仍待 R3 后用户裁决（I-006）。→ 已满足（2026-09-26）：用户指示 `docs/` 留给治理框架并授权编排器另规划路径，据此选定 `protocols/`（D-010），v1.0.0 全文升格并核对引用，工作区仅留指向存根（E-029）。
...
`progress: 100%` 由上方 3 个纲领阶段等权计算（已完成 3 / 3）。该值仅作展示，不放行阶段、不关闭 finding、不覆盖信息门禁，也不自动推导 `status: done`；Root 状态变更依据阶段退出条件与审计证据，不由 progress 单独推导。**Root 已于 2026-09-26 置 `done`**：依据为 R1–R3 阶段退出证据、[A-021](03-audit/A-021-root-closeout-review.md) 独立关门审计 `pass`（无 required finding）及 [A-022](03-audit/A-022-root-closeout-response.md) 响应；`progress` 100% 只是同源展示。
...
| I-006 | required | 协议指南经 R3 验证后的本仓共享运行文档承载路径与单一权威全文迁移方案。 | R3 升格、Root 关门 | R3 试跑验收与指南验证后、Root 关门前 | R2/R3 在 `attachments/consumer-response-protocol.md` 维护唯一草稿；R3 提供证据后提出具体共享路径与迁移方案，请用户确认，再迁移唯一来源并核对引用。 | verified | 用户于 2026-09-26 指示 `docs/` 保留给治理框架、由编排器另规划路径并评估与 VP-001 结果同置；据此选定仓库根 `protocols/`（D-010），v1.0.0 权威全文已升格、相对链接已改写、工作区仅留指向存根。复核触发：协议语义变化或承载路径变更。 | [D-010](01-decision/D-010-protocol-guide-promotion-path.md)、[E-029](02-execution/E-029-protocol-guide-promotion.md)、[`protocols/consumer-response-protocol.md`](../../../../protocols/consumer-response-protocol.md)。 |
