...
| R2 | 进行中；[E-008](02-execution/E-008-r2-readiness.md)；I-001/I-002 verified | 参与、仓库、责任人、授权与最小留存边界已确认；按 [D-003](01-decision/D-003-protocol-guide-lifecycle-and-promotion.md) 在 `attachments/consumer-response-protocol.md` 编写唯一草稿，完成 self + independent Reviewer cross 审计，再取得消费方可读可执行确认；草稿已形成，A-006/A-007 均为 fail；本次按 D-004 修正后待复审及消费方确认，5 条 required findings 保持 open；按 D-005 纳入下游交付选择权与承诺/写入前确认，本次具体目录随 I-003 明确。 |
| R3 | 待 R2 退出后登记 | 对一条真实且获授权处理的需求执行端到端试跑（I-003）；按 D-004 在 method-engineering 根 `runtime-records/<work-item-id>/` 承载 record.md 与追加式 events.md，I-003 真实需求授权后才分配实际 work-item ID，不在 WorldModel 克隆创建记录；修正歧义或登记有界 residual，试跑验收与指南验证后提出具体共享路径迁移方案，取得用户裁决并完成唯一全文升格和引用核对（I-006），在 Root 关门前形成 VP-002 关门证据；审计模式待真实需求范围明确后重新判定。 |
...
| R3 实际建档与逐案协作时（非阶段门禁） | I-005（字段、模板、work-item ID、引用、渠道） | non-blocking / open；D-004 只确定试点宿主，其余逐案确认 |
