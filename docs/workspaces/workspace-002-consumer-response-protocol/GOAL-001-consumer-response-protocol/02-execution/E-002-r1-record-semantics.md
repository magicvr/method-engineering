...
> 本条「已发生」与「截至本条初稿形成时未完成」记录提交前的初稿状态，不代表 checkpoint `7014f24` 形成时的完整状态。当前状态以 `00-meta.md`、执行/审计索引及 E-003 后续记录为准。
...
该 checkpoint 已包含 self 审计 A-001，并曾将 I-004 标为 `verified`；这与上述初稿时点不同。该过早验证状态后来在 A-002 响应过程中纠正为 `collecting`，见 E-003。保留初稿事实不表示认可 checkpoint 当时的门禁判断。
